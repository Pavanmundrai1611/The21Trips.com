...
import './SeaterBooking.css'; // Import the stylesheet
    ...
    if (selectedSeats.length === noOfSeatsToBeBooked) {
      ...
      try {
        ...
        if (sd && selectedPlan && busCode && selectedDate) {
          ...
          console.error('One or more necessary variables are undefined.');
        }
      } catch (error) {
        ...
        {customerNames.map((name, index) => (
          <div key={index}>
            <input
              ...
              required // make each customer name input mandatory
            />
          </div>
        ))}
        <button onClick={addCustomerNameInput} className='btn btn-outline-dark'>Add Customer Names</button>
      </div>
      ...
      <div className="seater">
        <table>
          ...
          <tr>
            ...
            <td></td>
            <td className="number number-31">31</td>
            <td className="number number-32">32</td>
          </tr>
        </table>
      </div>
